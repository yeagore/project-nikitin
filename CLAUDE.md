...
| Magicks | `Magicks` | The magickal density byte, grown rather than sampled: a Turing reaction (Gray–Scott) between the magick, which makes more of itself, and the inhibitor it feeds on, which is replenished everywhere and spreads faster — so the field breaks into spots, worms, mazes or lace instead of settling flat. Its coefficients are not knobs: the settings that pattern at all are islands in a sea of dead and flooded ones, so six of them are named as `MagickPattern` (motes, wells, veins, labyrinth, lace, hollows) and the stage shows two parameters — which pattern, and `MagickDensity`, how much magick the Domain holds. The reaction runs on its own lattice, four ground cells to the side, and is enlarged back onto the columns, so a feature is four cells across for every cell it would have been — magick is a place, not a texture. The lattice leans: the magick spreads more readily uphill, upwind and upstream and the aether the other way, so the same pattern carries more of itself on the tops, the weather side and the headwaters. Read by nothing. |
